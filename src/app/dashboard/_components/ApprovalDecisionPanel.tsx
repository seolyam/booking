...
export default function ApprovalDecisionPanel({
    ...
    const isReviewable = budgetStatus === "verified" || budgetStatus === "verified_by_reviewer";
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-8 shadow-sm">
            ...
            <div className="space-y-4 mb-8">
                ...
                <button
                    ...
                    className={`w-full p-4 rounded-xl border-2 transition-all text-left ${selectedAction === "approve"
                            ? "border-green-500 bg-green-50/30"
                            : "border-gray-100 hover:border-green-200"
                        } ${!isReviewable ? "opacity-50 cursor-not-allowed" : ""}`}
                >
                    <div className="flex items-start gap-3">
                        <CheckCircle className={`h-5 w-5 mt-0.5 ${selectedAction === "approve" ? "text-green-600" : "text-gray-400"}`} />
                        <div>
                            <div className="font-bold text-gray-900">Approve</div>
                            <div className="text-xs text-gray-500 font-medium">Grant final approval</div>
                        </div>
                    </div>
                </button>
                ...
                <button
                    ...
                    disabled={!isReviewable}
                    className={`w-full p-4 rounded-xl border-2 transition-all text-left ${selectedAction === "request_info"
                            ? "border-orange-400 bg-orange-50/30"
                            : "border-gray-100 hover:border-orange-200"
                        } ${!isReviewable ? "opacity-50 cursor-not-allowed" : ""}`}
                >
                    <div className="flex items-start gap-3">
                        <Info className={`h-5 w-5 mt-0.5 ${selectedAction === "request_info" ? "text-orange-500" : "text-gray-400"}`} />
                        <div>
                            <div className="font-bold text-gray-900">Request info</div>
                            <div className="text-xs text-gray-500 font-medium">Send to reviewer</div>
                        </div>
                    </div>
                </button>
                ...
                <button
                    ...
                    disabled={!isReviewable}
                    className={`w-full p-4 rounded-xl border-2 transition-all text-left ${selectedAction === "reject"
                            ? "border-red-500 bg-red-50/30"
                            : "border-gray-100 hover:border-red-200"
                        } ${!isReviewable ? "opacity-50 cursor-not-allowed" : ""}`}
                >
                    <div className="flex items-start gap-3">
                        <XCircle className={`h-5 w-5 mt-0.5 ${selectedAction === "reject" ? "text-red-600" : "text-gray-400"}`} />
                        <div>
                            <div className="font-bold text-gray-900">Reject</div>
                            <div className="text-xs text-gray-500 font-medium">Decline this request</div>
                        </div>
                    </div>
                </button>
            </div>
            ...
            <div className="mb-6">
                ...
                <Textarea
                    ...
                    className="w-full resize-none border-gray-200 rounded-xl focus:ring-[#358334]/20 focus:border-[#358334] min-h-[120px]"
                />
            </div>
