...
import { Search, LogOut, Trash2, Building2, Phone, Clock } from "lucide-react";
  ...
  return (
    <div className="-m-4 md:-m-8 p-4 md:p-8 w-full mx-auto flex flex-col min-h-[calc(100vh-theme(spacing.16))]">
      ...
      <div className="md:hidden space-y-3">
        {filteredLogs.length === 0 ? (
          <div className="py-16 text-center text-sm text-gray-500">
            {deferredQuery.trim() ? "No logs match your search." : "No visitor logs yet. Logs are created automatically when users sign in."}
          </div>
        ) : (
      ...
      <div className="hidden md:flex items-center justify-between mb-8">
        <div>
          <p className="text-sm text-gray-500">
            {filteredLogs.length} total &middot; {activeCount} currently active &middot; Logs are recorded automatically on login/logout
          </p>
        </div>
      </div>
          ...
          <table className="w-full min-w-[900px] text-left border-collapse">
            <thead>
              <tr className="border-b border-gray-100 text-[11px] uppercase tracking-wider text-gray-400">
                <th className="py-6 pl-8 pr-4 font-semibold w-[200px]">Name</th>
